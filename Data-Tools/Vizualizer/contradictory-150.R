...
data_dir <- 'Paper_Data/Contradictory-150/'
...
over_time_ac = ggplot(lines_ac, aes(x=eval, y=mean, group = pop_size, fill = pop_size, color = pop_size, shape = pop_size)) +
  ...
  geom_line(size = 0.5) +
  geom_point(data = filter(lines_ac, (eval %% 34000000 == 0 & eval != 0) | eval == 1500000000), size = 1.5, stroke = 2.0, alpha = 1.0) +
  scale_y_continuous(
    name="Coverage",
    limits=c(0, 100),
    breaks=seq(0,100, 25),
...
over_time_sc = ggplot(lines_sc, aes(x=eval, y=mean, group = pop_size, fill = pop_size, color = pop_size, shape = pop_size)) +
  geom_ribbon(aes(ymin = min, ymax = max), alpha = 0.1) +
  geom_line(size = 0.5) +
  geom_point(data = filter(lines_sc, (eval %% 34000000 == 0 & eval != 0) | eval == 1500000000), size = 1.5, stroke = 2.0, alpha = 1.0) +
  scale_y_continuous(
    name="Coverage",
    limits=c(0, 100),
    breaks=seq(0,100, 25),
...
fig = plot_grid(
  ggdraw() + draw_label("Results for contradictory objectives with 50 redundant cases", fontface='bold', size = 24) + p_theme,
  base,
  legend,
  nrow = 3,
  rel_heights = c(0.08,1.5,.06),
  labels = c('','  ','    '),
  label_size = TSIZE)

save_plot(
  filename ="contradictory-150.pdf",
  fig,
  base_width=13,
  base_height=10
)