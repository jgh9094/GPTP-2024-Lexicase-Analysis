...
```{r con-500-data-setup, message=FALSE}
library(ggplot2)
library(cowplot)
library(dplyr)
...
# over time data
over_time <- read.csv("../Paper_Data/Contradictory-500/ot.csv", header = TRUE, stringsAsFactors = FALSE)
over_time$pop_size <- factor(over_time$pop_size, levels = NAMES)

# best performance data
best <- read.csv('../Paper_Data/Contradictory-500/best.csv', header = TRUE, stringsAsFactors = FALSE)
...
```{r con-500-act-ot, message=FALSE, out.width="100%"}
...
```{r con-500-sat-ot, message=FALSE, out.width="100%"}
# aggregate
lines = over_time %>%
  group_by(pop_size, eval) %>%
  dplyr::summarise(
...
```{r con-500-sat-best, message=FALSE, out.width="100%"}
...
```{r con-500-sat-best-sts, message=FALSE, out.width="100%"}
best %>%
  group_by(pop_size) %>%
  dplyr::summarise(
    count = n(),
...
```{r con-500-sat-best-kru, message=FALSE, out.width="100%"}
...
```{r con-500-sat-best-wil, message=FALSE, out.width="100%"}
