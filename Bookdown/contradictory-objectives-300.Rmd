...
```{r con-300-data-setup, message=FALSE}
library(ggplot2)
library(cowplot)
library(dplyr)
...
```{r con-300-act-ot, message=FALSE, out.width="100%"}
# aggregate
lines = over_time %>%
...
```{r con-300-sat-ot, message=FALSE, out.width="100%"}
# aggregate
lines = over_time %>%
  group_by(pop_size, eval) %>%
  dplyr::summarise(
...
```{r con-300-sat-best, message=FALSE, out.width="100%"}
...
```{r con-300-sat-best-sts, message=FALSE, out.width="100%"}
best %>%
  group_by(pop_size) %>%
  dplyr::summarise(
    count = n(),
...
```{r con-300-sat-best-kru, message=FALSE, out.width="100%"}
...
```{r con-300-sat-best-wil, message=FALSE, out.width="100%"}
